...
void HCalcolo_costi::get_ricetta()
{
    ...
    sql ="SELECT prodotti.descrizione as 'MATERIALE',righe_ricette.quantita as \"QUANTITA\'\",prodotti.prezzo as 'COSTO UNITARIO (€)',righe_ricette.quantita*prodotti.prezzo as 'COSTO PER RICETTA',(prodotti.prezzo  * righe_ricette.quantita)* :f as 'COSTO FORMATO' FROM righe_ricette,prodotti,ricette WHERE righe_ricette.ID_ricetta=ricette.ID and prodotti.ID=righe_ricette.ID_prodotto and ricette.ID=(SELECT ID from ricette where ricette.ID_prodotto=:idp) group by prodotti.ID,ricette.ID,righe_ricette.ID";
    q.prepare(sql);
    ...
    for (int row = 0; row < r_rowCount; row++) {
        ...
        for (int column = 0; column < r_columnCount; column++) {
            if (!ui->tvRicetta->isColumnHidden(column)) {
                ...
                {
                    double res=0.0;
                    res=ui->tvRicetta->model()->data(ui->tvRicetta->model()->index(row, 3)).toDouble();
