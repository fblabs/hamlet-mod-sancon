...
void HWarehouseDetails::getLotdefData()
{
  ...
  QSqlQuery q(db);
  QString sql= "SELECT lotdef.lot_fornitore,lotdef.giacenza,lotdef.scadenza,lotdef.EAN FROM lotdef,operazioni WHERE lotdef.ID=operazioni.IDlotto AND operazioni.ID=:idop";
  q.prepare(sql);
  ...
  if (qmdl->index(0,2).data(0).toDate()>ui->deScadenza->minimumDate())
  {
    ui->label_11->setText("Scadenza:");
    ui->deScadenza->setVisible(true);
    ui->deScadenza->setDate(qmdl->index(0,2).data(0).toDate());
  }
  else
  {
  ...
  ui->leEAN->setText(qmdl->index(0,3).data(0).toString());
